...
def train(args):
    ...
    if args.restore_ckpt is not None:
        # 为什么还没开始训练就让我提供模型参数，那怎么在自己的数据集上训练呢？难道这样可以使用预训练模型
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            ...
            # 滑动窗口拆分图像
            image1_wids = create_windows_4(image1)  # torch.Size([18, 3, 184, 248])
            image2_wids = create_windows_4(image2)

            if args.confidence:  # 进行置信度估计
                flow_predictions, flow_confidence = model(image1_wids, image2_wids, iters=args.iters)
                flow_pred_combined = combine_windows_flow_4(flow_predictions)
                loss, metrics, flow_loss, conf_loss = sequence_loss_conf(flow_pred_combined, flow,
                                                                         flow_confidence, valid, args.gamma)
            else:  # 原始网络，不进行置信度估计
                flow_predictions = model(image1_wids, image2_wids, iters=args.iters)
                flow_pred_combined = combine_windows_flow_4(flow_predictions)
                loss, metrics = sequence_loss(flow_pred_combined, flow, valid, args.gamma)
            ...
            if total_steps % VAL_FREQ == VAL_FREQ - 1:
                PATH = 'checkpoints-sliding-window/%d_%s.pth' % (total_steps+1, args.name)
    ...
    logger.close()
    PATH = 'checkpoints-sliding-window/%s.pth' % args.name
    torch.save(model.state_dict(), PATH)
...
if __name__ == '__main__':
    ...
    if not os.path.isdir('checkpoints-sliding-window'):
        os.mkdir('checkpoints-sliding-window')
